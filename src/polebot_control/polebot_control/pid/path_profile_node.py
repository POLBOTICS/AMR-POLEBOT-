...
class PathProfileNode(Node):
    ...
    def generate_path_points(self) -> List[Tuple[float, float]]:
        ...
        if mode == 'straight':
            ...
            # Lemniscate of Bernoulli (Infinity Logo)
            # R_min = a * sqrt(2) / 3  =>  a = turn_radius_1 * 3 / sqrt(2)
            # Parametric:
            #   x(t) = a * sqrt(2) * cos(t) / (1 + sin(t)^2)
            #   y(t) = - a * sqrt(2) * sin(t) * cos(t) / (1 + sin(t)^2)
            # for t in [-pi/2, 3*pi/2]
            # Traverses: (0,0) [hdg=45 deg] -> right lobe -> (0,0) [hdg=135 deg] -> left lobe -> (0,0) [hdg=45 deg]
            turn_radius = max(self.turn_radius_1, 0.5)
            a = turn_radius * 3.0 / math.sqrt(2.0)

            n_dense = 2000
            t_vals = [(-0.5 * math.pi) + (2.0 * math.pi * i / (n_dense - 1)) for i in range(n_dense)]
            ...
            for t in t_vals:
                ...
                px = (a * math.sqrt(2.0) * cos_t) / denom
                py = -(a * math.sqrt(2.0) * sin_t * cos_t) / denom
                if abs(px) < 1e-9:
                    px = 0.0
                if abs(py) < 1e-9:
                    py = 0.0
                dense_pts.append((px, py))
            ...
            for i in range(1, n_dense):
                ...
                s_dense.append(s_dense[-1] + math.hypot(dx, dy))
            total_len = s_dense[-1]

            # Resample uniformly with samples_per_meter
            step = 1.0 / max(self.samples_per_meter, 2)
            n_samples = max(int(math.ceil(total_len / step)), 20)

            resampled_points: List[Tuple[float, float]] = []
            curr_idx = 0
            for i in range(n_samples + 1):
                target_s = total_len * (i / n_samples)
                while curr_idx < n_dense - 1 and s_dense[curr_idx + 1] < target_s:
                    curr_idx += 1
                if curr_idx >= n_dense - 1:
                    resampled_points.append(dense_pts[-1])
                else:
                    ds_seg = s_dense[curr_idx + 1] - s_dense[curr_idx]
                    ratio = 0.0 if abs(ds_seg) < 1e-9 else (target_s - s_dense[curr_idx]) / ds_seg
                    x_interp = dense_pts[curr_idx][0] + ratio * (dense_pts[curr_idx + 1][0] - dense_pts[curr_idx][0])
                    y_interp = dense_pts[curr_idx][1] + ratio * (dense_pts[curr_idx + 1][1] - dense_pts[curr_idx][1])
                    if abs(x_interp) < 1e-9:
                        x_interp = 0.0
                    if abs(y_interp) < 1e-9:
                        y_interp = 0.0
